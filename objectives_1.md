...
4. Definir y calcular un conjunto de métricas objetivas y perceptuales para la evaluación de los modelos, distinguiendo entre un bloque bibliográfico (métricas ya publicadas, como PESQ, STOI, SI-SDR y LSD) y un bloque empírico sobre audio real sin referencia limpia (métricas no intrusivas, como DNSMOS y NISQA), complementado con un barrido controlado de audios sintéticos degradados que permite emplear también métricas intrusivas allí donde el audio real carece de referencia limpia.
...
8. Desarrollar una aplicación interactiva mediante Gradio, con una interfaz de dos niveles (categoría de degradación → modelo o baseline), que permita cargar audio, aplicar el modelo seleccionado y visualizar los resultados (audio antes/después, espectrogramas comparativos y métricas).

...
Fix (corte de audio al final): AudioSR procesa cada chunk con ventanas de
duración fija en su proceso de difusión, así que un chunk que no llena esa
ventana (típicamente el último, casi nunca de exactamente CHUNK_SECONDS)
puede devolver un waveform más corto o más largo de lo esperado. Si no se
corrige, esa diferencia de longitud se arrastra en la concatenación final y
se traduce en pérdida de cola de audio. Se normaliza cada chunk_out a su
target_len exacto (recorte o padding por repetición de borde) antes de
añadirlo a chunks_out.
...
def procesar(ruta_audio: str, ruta_salida: str):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        for i, start in enumerate(starts):
            ...
            # Longitud exacta que debe tener este chunk a la salida (48kHz),
            # calculada a partir de su duración real de entrada -- es el
            # valor de referencia contra el que se normaliza chunk_out más
            # abajo, para que AudioSR nunca pueda descuadrar la duración
            # total del audio final.
            target_len = round((end - start) * SR_SALIDA / sr_in)
            ...
            # --- Normalización de longitud: evita que un chunk más corto o
            # más largo de lo esperado (típico en AudioSR con chunks que no
            # llenan su ventana interna de difusión) descuadre la duración
            # total del audio final y "corte" la cola del resultado. ---
            if chunk_out.shape[1] < target_len:
                chunk_out = np.pad(
                    chunk_out, ((0, 0), (0, target_len - chunk_out.shape[1])), mode="edge"
                )
            elif chunk_out.shape[1] > target_len:
                chunk_out = chunk_out[:, :target_len]
